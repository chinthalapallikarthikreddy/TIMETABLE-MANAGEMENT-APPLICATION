...
def test_branch_empty_list(tmp_path):
    export_fn = _get_export_fn()
    out = os.path.join(tmp_path, "export_empty.txt")
    export_fn([], out)
    assert os.path.exists(out)


def test_branch_non_empty_list(tmp_path):
    export_fn = _get_export_fn()
    out = os.path.join(tmp_path, "export_non_empty.txt")
